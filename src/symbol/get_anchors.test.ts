...
describe('getAnchors', () => {
    ...
    test('non-continued line with short labels', () => {
        ...
        expect(labelLength / 2 + 1 <= anchors[0].y && anchors[0].y < labelLength / 2 + 3 * glyphSize + 1).toBeTruthy();
    });
    ...
    test('use middle point as a fallback position for short non-continued lines', () => {
        ...
        expect(anchors).toEqual([{x: 1, y: 2, angle: 1.5707963267948966, segment: 0}]);
    });
