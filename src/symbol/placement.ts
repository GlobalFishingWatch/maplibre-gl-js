...
} from '../style/style_layer/symbol_style_layer_properties';
...
function calculateVariableLayoutShift(
    ...
    return new Point(shiftX + offset[0] * textBoxScale, shiftY + offset[1] * textBoxScale);
}

function shiftVariableCollisionBox(
...
export class Placement {
    ...
    placeLayerBucketPart(
        ...
        const placeSymbol = (symbolInstance: SymbolInstance, collisionArrays: CollisionArrays) => {
                ...
                if (!layout.get('text-variable-anchor')) {
                    ...
                    const placeVertical = () => {
                        ...
                        if (
                            ...
                            return placeBoxForVariableAnchors(
                                verticalTextBox,
            ...
            if (symbolInstance.useRuntimeCollisionCircles) {
                ...
                placeText =
                    textAllowOverlap ||
                    (placedGlyphCircles.circles.length > 0 && !placedGlyphCircles.collisionDetected);
            ...
            if (collisionArrays.iconBox) {
                const placeIconFeature = iconBox => {
                    const shiftedIconBox =
                        hasIconTextFit && shift
                ...
                if (
                    placedVerticalText &&
                    placedVerticalText.box &&
                    placedVerticalText.box.length &&
                    collisionArrays.verticalIconBox
                ) {
    ...
    markUsedOrientation(bucket: SymbolBucket, orientation: number, symbolInstance: SymbolInstance) {
        const horizontal =
            orientation === WritingMode.horizontal || orientation === WritingMode.horizontalOnly ? orientation : 0;
        ...
        if (symbolInstance.verticalPlacedTextSymbolIndex) {
            bucket.text.placedSymbolArray.get(symbolInstance.verticalPlacedTextSymbolIndex).placedOrientation =
                vertical;
        }
    }
    ...
    updateBucketOpacities(
        ...
        if (
            ...
            collisionBoxArray &&
            (bucket.hasIconCollisionBoxData() || bucket.hasTextCollisionBoxData())
        ) {
            bucket.deserializeCollisionBoxes(collisionBoxArray);
        }
        ...
        for (let s = 0; s < bucket.symbolInstances.length; s++) {
            ...
            const verticalHidden =
                placedOrientation === WritingMode.horizontal || placedOrientation === WritingMode.horizontalOnly;
            ...
            if (hasText) {
                ...
                if (symbolInstance.verticalPlacedTextSymbolIndex >= 0) {
                    bucket.text.placedSymbolArray.get(symbolInstance.verticalPlacedTextSymbolIndex).hidden =
                        symbolHidden || verticalHidden ? 1 : 0;
                }
            ...
            if (hasIcon) {
                ...
                const useHorizontal = !(
                    hasIconTextFit &&
                    symbolInstance.verticalPlacedIconSymbolIndex &&
                    horizontalHidden
                );
                    ...
                    if (collisionArrays.textBox || collisionArrays.verticalTextBox) {
                        ...
                        if (collisionArrays.textBox) {
                            updateCollisionVertices(
                                bucket.textCollisionBox.collisionVertexArray,
                                opacityState.text.placed,
                                !used || horizontalHidden,
                                shift.x,
                                shift.y
                            );
                        }
...
function updateCollisionVertices(
    collisionVertexArray: CollisionVertexArray,
    placed: boolean,
    notUsed: boolean | number,
    shiftX?: number,
    shiftY?: number
) {
