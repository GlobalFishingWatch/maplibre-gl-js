...
function createFilter(filter: any): FeatureFilter {
    ...
    if (compiled.result === 'error') {
        ...
        return {
            filter: (globalProperties: GlobalProperties, feature: Feature, canonical?: CanonicalTileID) =>
...
function convertFilter(filter?: Array<any> | null): unknown {
    ...
    const converted =
        op === '=='
            ...
            : op === 'none'
            ? ['all' as unknown].concat(filter.slice(1).map(convertFilter).map(convertNegation))
            : op === 'in'
