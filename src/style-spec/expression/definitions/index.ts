...
CompoundExpression.register(expressions, {
    ...
    'filter-type-==': [BooleanType, [StringType], (ctx, [v]) => ctx.geometryType() === (v as any).value],
    'filter-<': [
        BooleanType,
        [StringType, ValueType],
        (ctx, [k, v]) => {
    ...
    'filter-id-in': [BooleanType, [array(ValueType)], (ctx, [v]) => (v as any).value.indexOf(ctx.id()) >= 0],
    'filter-in-small': [
        BooleanType,
    ...
    'filter-in-large': [
        ...
        (ctx, [k, v]) =>
            binarySearch(ctx.properties()[(k as any).value], (v as any).value, 0, (v as any).value.length - 1)
    ],
    all: {
        type: BooleanType,
        overloads: [
